...
Let's look the first summary of this object.
The output shows that objects with class `sf` generally behave like a regular `data.frame`: all but the last column was summarised as they would be in a non-geographical data frame.
he inclusion of additional geographical data (`geometry type`, `dimension`, `bbox` and CRS information - `espf (SRID)`, `proj4string`), and the presence of a 4^th^ `geometry` column (shown):
...
The summary for this final column show 3 things that are special about spatial data: it has a 
